...
def convert_examples_to_features(item):
    ...
    levels = findIndex(js['levels'])
    sample_count = args.sample_count

    function_tokens = []
    function_ids = []
    if (levels[0] != -1):
        for func in js['function']:
    ...
    function_ids = function_ids[:max_functions]
    function_ids+=[[tokenizer.pad_token_id]*args.context_length]*(max_functions-len(function_ids))

    if len(js['function']) < 1:
        routes = [[-1] * max_functions] * sample_count
    else:
        routes = sample(function_ids, levels, sample_count)
    up_fun_num = js['up_fun_num']

    return InputFeatures(code_ids,position_idx,dfg_to_code,dfg_to_dfg,nl_ids,js['url'], package_ids, function_ids, levels, routes, up_fun_num)
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item): 
        ...
        return (torch.tensor(self.examples[item].code_ids),
              ...
              torch.tensor(self.examples[item].routes), torch.tensor(self.examples[item].up_fun_num))
...
def train(args, model, tokenizer,pool):
    ...
    for idx in range(args.num_train_epochs): 
        for step,batch in enumerate(train_dataloader):
            ...
            package_inputs = batch[4].to(args.device)
            function_inputs = batch[5].to(args.device)
            levels = batch[6]
            routes = batch[7]
            #get code and nl vectors
            code_vec = model(code_inputs=code_inputs,attn_mask=attn_mask,position_idx=position_idx)
            nl_vec = model(nl_inputs=nl_inputs, context_inputs=[package_inputs, function_inputs], levels=levels, routes=routes)
        ...
        checkpoint_prefix = 'checkpoint-history'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))                        
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)                        
        model_to_save = model.module if hasattr(model,'module') else model
        output_dir = os.path.join(output_dir, '{}'.format('model' + str(idx) + '.bin')) 
...
def evaluate(args, model, tokenizer,file_name,pool, eval_when_training=False):
    ...
    code_dataset = TextDatasetBase(tokenizer, args, args.codebase_file, pool)
    ...
    for batch in query_dataloader:  
        ...
        up_fun_num = batch[8]
        with torch.no_grad():
            nl_vec = model(nl_inputs = nl_inputs, context_inputs=[package_inputs, function_inputs], levels=levels, up_fun_num=up_fun_num)
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--train_data_file", default="./data/python/example_train.json", type=str, required=False,
                        help="The input training data file (a json file).")
    parser.add_argument("--output_dir", default="./saved_models/python", type=str, required=False,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--eval_data_file", default="./data/python/example_valid.json", type=str,
                        help="An optional input evaluation data file to evaluate the MRR(a jsonl file).")
    parser.add_argument("--test_data_file", default="./data/python/example_test.json", type=str,
                        help="An optional input test data file to test the MRR(a josnl file).")
    parser.add_argument("--codebase_file", default="./data/python/example_codebase.json", type=str,
    ...
    parser.add_argument("--context_length", default=20, type=int,
                        help="Optional Code input sequence length after tokenization.") 
    ...
    parser.add_argument("--sample_count", default=2, type=int, help="the sample count hyper-parameter m")
    ...
    if args.do_train:
        ...
        if os.path.exists(os.path.join(args.output_dir, checkpoint_prefix)):
